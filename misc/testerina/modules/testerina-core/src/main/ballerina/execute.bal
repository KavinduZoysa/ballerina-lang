...
function executeTest(TestFunction testFunction) {
    ...
    testFunction.groups.forEach(g => groupStatusRegistry.incrementExecutedTest(g));
...
function executeBeforeGroupFunctions(TestFunction testFunction) {
    foreach string g in testFunction.groups {
        TestFunction[]? beforeGroupFunctions = beforeGroupsRegistry.getFunctions(g);
        if beforeGroupFunctions != () && !groupStatusRegistry.firstExecuted(g) {
            ExecutionError? err = executeFunctions(beforeGroupFunctions, shouldSkip);
            if err is ExecutionError {
                testFunction.skip = true;
                groupStatusRegistry.setSkipAfterGroup(g);
                exitCode = 1;
...
function executeAfterGroupFunctions(TestFunction testFunction) {
    foreach string g in testFunction.groups {
        TestFunction[]? afterGroupFunctions = afterGroupsRegistry.getFunctions(g);
        if afterGroupFunctions != () && groupStatusRegistry.lastExecuted(g) {
            ExecutionError? err = executeFunctions(afterGroupFunctions,
                shouldSkip || groupStatusRegistry.getSkipAfterGroup(g));
            if err is ExecutionError {
                exitCode = 1;
